...
CREATE TABLE IF NOT EXISTS `servlab_shopnv`.`employees` (
  ...
  PRIMARY KEY (`id`, `persons_id`, `users_id`),
...
ALTER TABLE `servlab_shopnv`.`employees`
  ADD COLUMN `departments_id` INT(11) NOT NULL
  AFTER `users_id`,
  DROP PRIMARY KEY,
  ADD PRIMARY KEY (`id`, `persons_id`, `users_id`, `departments_id`),
  ADD INDEX `fk_employees_departments1_idx` (`departments_id` ASC);
ALTER TABLE `servlab_shopnv`.`employees`
  ADD CONSTRAINT `fk_employees_departments1`
FOREIGN KEY (`departments_id`)
REFERENCES `servlab_shopnv`.`departments` (`id`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;

ALTER TABLE `servlab_shopnv`.`persons`
  DROP COLUMN `i_name`,
  ADD COLUMN `i_name` VARCHAR(20) NOT NULL
  AFTER `f_name`;
